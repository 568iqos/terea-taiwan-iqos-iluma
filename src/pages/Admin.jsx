    ...
    <div style={styles.container}>
      ...
      {activeTab === 'products' && (
        <div style={styles.tabContent}>
          <h2>產品圖片與名稱管理</h2>
          <p style={styles.instruction}>點擊產品名稱可編輯。點擊「選圖」上傳圖片。</p>
              ...
              <div key={product.id} style={styles.productCard}>
                ...
                <div style={styles.uploadSection}>
                  <input 
                    type="file" 
                    accept="image/*" 
                    onChange={(e) => handleImageSelect(product.id, e.target.files[0])} 
                    style={styles.fileInput} 
                    id={`image-${product.id}`} 
                  />
                  <label htmlFor={`image-${product.id}`} style={styles.uploadLabel}>
                    {imageInput[product.id]?.preview ? '✓ 已選圖' : '⬆ 選圖'}
                  </label>
                  {imageInput[product.id] && (
                    <button onClick={() => uploadProductImage(product.id)} style={styles.confirmUploadBtn} disabled={loading}>
                      {loading ? '上傳中...' : '確認上傳'}
                    </button>
                  )}
                </div>
